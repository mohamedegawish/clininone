@extends('layouts.app')

@section('title', 'إضافة عيادة جديدة')

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">إضافة عيادة للنظام</h1>
        <p class="page-subtitle">تسجيل عيادة جديدة وبدء الإدارة</p>
    </div>
</div>

<div class="card">
    <div class="card-header">
        <h3 class="card-title">بيانات العيادة</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.clinics.store') }}" method="POST">
            @csrf
            
            <div class="form-grid">
                <div class="form-group span-2">
                    <label class="form-label">اسم العيادة <span class="req">*</span></label>
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="مثلاً: عيادة الأمل">
                </div>

                <div class="form-group span-2">
                    <label class="form-label">العنوان <span class="req">*</span></label>
                    <input type="text" name="address" class="form-control" value="{{ old('address') }}" required placeholder="العنوان بالتفصيل">
                </div>

                <div class="form-group">
                    <label class="form-label">حالة العيادة <span class="req">*</span></label>
                    <select name="status" class="form-control" required>
                        <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>نشطة</option>
                        <option value="inactive" {{ old('status', 'inactive') == 'inactive' ? 'selected' : '' }}>غير نشطة</option>
                    </select>
                </div>
            </div>

            <div class="d-flex justify-content-end mt-24">
                <button type="submit" class="btn btn-primary btn-lg px-40">
                    <i class="ph-bold ph-check"></i>
                    <span>إنشاء العيادة</span>
                </button>
            </div>
        </form>
    </div>
</div>
@endsection
