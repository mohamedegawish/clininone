/**
 * booking.js — Booking Logic (Vanilla JS matching Angular layout)
 */
...
let visitType = 'new';
let selectedSlot = '';
let form = { patientName: '', patientPhone: '', patientAge: '', patientGender: 'Male', notes: '' };
let loading = false;
let toastMsg = null;
...
function updateBackLink(docId) {
  ...
  if (link) link.href = `doctor.html?id=\${docId}`;
}
...
window.nextStep = function() {
  ...
  if (step === 2) {
    ...
    form.patientAge = fa;
    form.patientGender = document.getElementById('fg').value;
...
window.confirmBooking = async function() {
  ...
  try {
    ...
    // For simplicity, just redirect to home on success
...
function render() {
  ...
  const effectivePrice = visitType === 'followup' ? Math.round(doctor.price * 0.6) : doctor.price;
  ...
  let html = `
    ...
    <div class="booking-doctor-card glass">
      ...
      <div>
        <div class="bk-doc-name">\${doctor.name}</div>
        <div class="specialty-tag">\${doctor.specialty}</div>
      </div>
      <div class="bk-doc-price">
        <span class="price-val">EGP \${doctor.price}</span>
      ...
      <div class="booking-card card animate-fade-in">
        ...
        <div class="visit-type-row">
          <button class="visit-type-btn \${visitType === 'new' ? 'active' : ''}" onclick="setVisitType('new')">
            ...
            <span class="vt-price">EGP \${doctor.price}</span>
          </button>
          <button class="visit-type-btn \${visitType === 'followup' ? 'active' : ''}" onclick="setVisitType('followup')">
            ...
            <span class="vt-price">EGP \${Math.round(doctor.price * 0.6)} <span class="vt-discount">-40%</span></span>
          </button>
        </div>
        \${visitType === 'followup' ? \`
          <div class="followup-note">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            الخصم يُطبَّق إذا كان لديك زيارة مكتملة خلال 30 يوماً.
          </div>
        \` : ''}
        ...
        <div class="slots-grid">
          \${slots.map(s => \`
            <button class="slot-btn \${selectedSlot === s ? 'selected' : ''}" onclick="selectSlot('\${s}')">\${s}</button>
          \`).join('')}
          \${slots.length === 0 ? '<p style="grid-column:1/-1;color:var(--text-muted);font-size:13px">لا يوجد مواعيد اليوم</p>' : ''}
        </div>
        <div class="step-actions">
          <a class="btn btn-ghost" href="doctor.html?id=\${doctor.id}">إلغاء</a>
...
async function init() {
  ...
  try {
    const [docData, docSlots] = await Promise.all([ fetchDoctor(id), fetchAvailableSlots(id, today) ]);
    doctor = docData;
    slots = docSlots;
    render();
  } catch (err) {
    document.getElementById('booking-container').innerHTML = '<div style="text-align:center;padding:50px;color:red">فشل تحميل البيانات.</div>';
  }
}
